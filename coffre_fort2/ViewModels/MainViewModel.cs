    ...
    public class MainViewModel : BaseViewModel
    {
        ...
        private async Task ChargerDepuisApiAsync()
        {
            try
            {
                ...
                await SauvegarderMotsDePasseEnLocalAsync();
        ...
        private bool EstEntreeValide(PasswordEntry entry)
        {
            return entry != null
                && !string.IsNullOrWhiteSpace(entry.NomApplication)
                && !string.IsNullOrWhiteSpace(entry.Identifiant)
                && !string.IsNullOrWhiteSpace(entry.MotDePasse)
                && entry.Tags != null
        ...
        private async void ImporterDepuisCsv()
        {
            ...
            if (openFileDialog.ShowDialog() == true)
            {
                try
                {
                    ...
                    using (var reader = new StreamReader(openFileDialog.FileName))
                    {
                        ...
                        while (!reader.EndOfStream)
                        {
                            ...
                            if (values.Length >= 4)
                            {
                                ...
                                {
                                    NomApplication = values[0],
                                    Identifiant = values[1],
                                    MotDePasse = values[2],
                                    Tags = new List<string> { values[3] },
                                    Utilisateur = _utilisateur,
                                    UtilisateurId = _utilisateurId
                                };

                                if (EstEntreeValide(entry))
                                {
                                    nouvellesEntrees.Add(entry);
                                }
                                else
                                {
                                    MessageBox.Show("Une ou plusieurs entrées CSV sont invalides. L'importation est annulée.", "Erreur", MessageBoxButton.OK, MessageBoxImage.Warning);
                                    return;
                                }
                            }
                            else
                            {
                                MessageBox.Show("Format CSV incorrect. L'importation est annulée.", "Erreur", MessageBoxButton.OK, MessageBoxImage.Warning);
                                return;
                            }
                        }
                    }

                    if (nouvellesEntrees.Count == 0)
                    {
                        MessageBox.Show("Aucune entrée valide trouvée dans le fichier CSV.", "Avertissement", MessageBoxButton.OK, MessageBoxImage.Warning);
                        return;
                    }
                    ...
                    MessageBox.Show("Import et sauvegarde terminé.", "Import CSV", MessageBoxButton.OK, MessageBoxImage.Information);
                }
                catch (Exception ex)
                {
        ...
        private async void ImporterDepuisXml()
        {
            ...
            if (openFileDialog.ShowDialog() == true)
            {
                try
                {
                    ...
                    if (nouvellesEntrees == null || nouvellesEntrees.Count == 0)
                    {
                        MessageBox.Show("Le fichier XML est vide ou invalide.", "Erreur", MessageBoxButton.OK, MessageBoxImage.Warning);
                        return;
                    }
                    ...
                    foreach (var entry in nouvellesEntrees)
                    {
                        ...
                        if (!EstEntreeValide(entry))
                        {
                            MessageBox.Show("Une ou plusieurs entrées XML sont invalides. L'importation est annulée.", "Erreur", MessageBoxButton.OK, MessageBoxImage.Warning);
                            return;
                        }
                    }
                    ...
                    MessageBox.Show("Import et sauvegarde terminé.", "Import XML", MessageBoxButton.OK, MessageBoxImage.Information);
                }
                catch (Exception ex)
                {
        ...
        private async Task SauvegarderMotsDePasseEnLocalAsync()
        {
            try
            {
                string dossierSauvegarde = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "CoffreFortBackups");
                Directory.CreateDirectory(dossierSauvegarde);

                var motsDePasse = await _passwordService.GetByUserIdAsync(_utilisateurId);

                string nomFichier = $"backup_{DateTime.Now:yyyyMMdd_HHmmss}.json";
                string cheminComplet = Path.Combine(dossierSauvegarde, nomFichier);

                var options = new JsonSerializerOptions { WriteIndented = true };
                string json = JsonSerializer.Serialize(motsDePasse, options);

                await File.WriteAllTextAsync(cheminComplet, json);

                Message = $"Sauvegarde automatique réalisée : {nomFichier}";
            }
            catch (Exception ex)
            {
                MessageBox.Show("Erreur lors de la sauvegarde locale : " + ex.Message);
            }
        }
