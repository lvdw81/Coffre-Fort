...
namespace coffre_fort_api.Controllers
{
    ...
    public class PasswordController : ControllerBase
    {
        ...
        public async Task<ActionResult<PasswordEntry>> Create([FromBody] PasswordEntry entry)
        {
            ...
            if (string.IsNullOrWhiteSpace(entry.NomApplication)
                || string.IsNullOrWhiteSpace(entry.Identifiant)
                || string.IsNullOrWhiteSpace(entry.MotDePasse)
                || entry.UserId == 0
                || entry.Tags == null
                || entry.Tags.Any(t => string.IsNullOrWhiteSpace(t)))
            {
                return BadRequest("Champs manquants ou invalides dans l'entree.");
            }
            ...
            {
                // Optionnel : loggue ici dbEx.InnerException?.Message
                return StatusCode(500, "Erreur de base de donnees lors de l'insertion.Vérifier les données");
            }
        }
